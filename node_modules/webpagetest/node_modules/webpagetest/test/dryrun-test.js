  ...
  describe('An Example WebPageTest Server', function() {
    ...
    it('gets a test results with extra data request', function(done) {
      wpt.getTestResults('141106_8N_ZRC', {
        ...
        if (err) return done(err);
        assert.equal(data.url, wptServer +
          'jsonResult.php?test=150809_G1_CAZ&breakdown=1&domains=1&pagespeed=1&requests=1');
        done();
      });
    });
    ...
    it('gets a waterfall image request', function(done) {
      wpt.getWaterfallImage('120816_V2_2', {dryRun: true}, function (err, data) {
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'waterfall.php?test=150809_G1_CAZ&run=1&cached=0');
        done();
      });
    });
    ...
    it('gets a waterfall thumbnail request', function(done) {
      wpt.getWaterfallImage('120816_V2_2', {
        ...
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'thumbnail.php?test=150809_G1_CAZ&run=1&cached=0&file=1_waterfall.png');
        done();
      });
    });
    ...
    it('gets a customized waterfall image request', function(done) {
      wpt.getWaterfallImage('120816_V2_2', {
        ...
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'waterfall.php?test=150809_G1_CAZ&run=1&cached=0&type=connection&mime=1&width=640&max=9&requests=1%2C2%2C4%2C6-8&cpu=0&bw=0&dots=0&labels=0');
        done();
      });
    });
    ...
    it('gets a screenshot request', function(done) {
      wpt.getScreenshotImage('120816_V2_2', {dryRun: true}, function (err, data) {
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'getgzip.php?test=150809_G1_CAZ&file=1_screen.jpg');
        done();
      });
    });
    ...
    it('gets a screenshot thumbnail request', function(done) {
      wpt.getScreenshotImage('120816_V2_2', {
        ...
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'thumbnail.php?test=150809_G1_CAZ&file=1_screen.jpg&run=1&cached=0');
        done();
      });
    });
    ...
    it('gets a screenshot in full resolution request', function(done) {
      wpt.getScreenshotImage('120816_V2_2', {
        ...
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'getgzip.php?test=150809_G1_CAZ&file=1_screen.png');
        done();
      });
    });
    ...
    it('gets a custom test request using aliases', function(done) {
      wpt.test('http://twitter.com/marcelduran', {
        ...
        if (err) return done(err);
        assert.equal(data.url, wptServer + 'runtest.php?url=http%3A%2F%2Ftwitter.com%2Fmarcelduran&fvonly=1&netlog=1&pngss=1&location=Local_Firefox_Chrome%3AChrome&runs=3&label=test%20123&timeline=1&r=150809_G1_CAZ&f=json');
        done();
      });
    });
    ...
    it('create a video', function (done) {
      wpt.createVideo('130416_YS_KD4-r:3-c:1,130416_W6_KEE-r:8-c:1', {dryRun: true}, function (err, data) {
        if (err) throw err;
        assert.equal(data.url, wptServer + 'video/create.php?tests=150809_G1_CAZ-r%3A3-c%3A1%2C130416_W6_KEE-r%3A8-c%3A1&f=json&end=visual');
        done();
      });
    });
    ...
    it('create a video with document complete comparison end point', function (done) {
      wpt.createVideo('130416_YS_KD4-r:3-c:1,130416_W6_KEE-r:8-c:1', {dryRun: true, comparisonEndPoint: 'doc'}, function (err, data) {
        if (err) throw err;
        assert.equal(data.url, wptServer + 'video/create.php?tests=150809_G1_CAZ-r%3A3-c%3A1%2C130416_W6_KEE-r%3A8-c%3A1&f=json&end=doc');
        done();
      });
    });
    ...
    it('get the url of an embedded video', function (done) {
      wpt.getEmbedVideoPlayer('130416_36ed6e37013655a14b2b857cdccec99db72adcaa', {dryRun: true}, function (err, data) {
        if (err) throw err;
        assert.equal(data.url, wptServer + 'video/view.php?embed=1&id=150809_c5b9ababcf2a54a7e3f9a26d7fe4eac8d8b4c84c');
        done();
      });
    });
